...
6. Write atomically: build the output into a temp file in the same directory, then `mv` it to `facts.jsonl` on success. Never modify `facts.md` — that is the operator's prerogative after they verify the result.
