...
## Collection Recipe
[Numbered steps for searching and reading data about the target person — used by profile-collector]

## Fact Extraction
[What signals to record, the URL format if any, and how to format the facts.md line]

## Discovery Recipe
[Numbered steps for scanning the source to find unprofiled people the user interacts with — used by profile-discovery. Optional; omit when the source has no person-discovery shape.]
...
**Consumers:** The same adapter file is read by both `profile-collector` (Collection Recipe) and `profile-discovery` (Discovery Recipe). Source-specific knowledge (MCP tools, identifiers, pitfalls) lives once, in one place.
